...
| `asteroid_hollowing` | Asteroid Hollowing | X15 | 16 MC | Tag power, **Promo**. Acción: gastar 1 titanio → +1 asteroide en la propia carta, +1 producción MC. Vocabulario existente, sin piezas nuevas |
| `comet_aiming` | Comet Aiming | X16 | 17 MC | Tag power, **Promo**. Acción con elección: gastar 1 titanio → +1 asteroide a CUALQUIER carta elegida (`target_card_resource_delta_allow_self`), O gastar 1 asteroide propio → coloca 1 océano. Vocabulario existente, sin piezas nuevas |
| `cutting_edge_technology` | Cutting Edge Technology | X18 | 12 MC | Tag science, **Promo**. Pasivo: -2 MC al jugar cualquier carta que tenga un `requirements` propio, sin importar sus tags (pieza nueva `requires_requirement` en `card_cost_discount_mc`, filtra por presencia de requisito en vez de por tag — `compute_card_cost_discount` suma el parámetro nuevo `has_requirement`, que `tools.play_card` calcula del catálogo) |
| `directed_impactors` | Directed Impactors | X19 | 8 MC | Tag power, **Promo**. Acción con elección: gastar 6 MC (titanio puede cubrir parte, `mc_or_titanium`) → +1 asteroide a CUALQUIER carta, O gastar 1 asteroide propio → +1 paso temperatura. Vocabulario existente, sin piezas nuevas |
| `field_capped_city` | Field-Capped City | X21 | 29 MC | Tags power+city+building, **Promo**. +2 producción MC, +1 producción energía, +3 plantas, coloca 1 ciudad REAL en el tablero (`city_hex_ids`, distinto de Stanford Torus que es fuera de mapa). Vocabulario existente |
...
| X17 | Crash Site Cleanup (Promo) | Mismo motivo que Law Suit: su REQUISITO de juego es "a player removed another player's plants this generation" -- nunca observable en single-player. |
